...
import { Clock, AlertTriangle, CheckCircle2, CalendarIcon, Filter, X } from 'lucide-react';
...
export default function InlineTasksPanel({ onOpenVisit }: Props) {
  ...
  const applyFilters = (tasks: TaskItem[]) => {
    let result = tasks;
    if (filterUser !== 'all') result = result.filter(t => t.visit.userId === filterUser);
    if (filterPartner !== 'all') result = result.filter(t => t.visit.partnerId === filterPartner);
    if (dateRange?.from) {
    ...
    if (filterOverdue === 'overdue') result = result.filter(t => getDaysPending(t.task.createdAt) >= 10);
    else if (filterOverdue === 'on_time') result = result.filter(t => getDaysPending(t.task.createdAt) < 10);
    return result;
  };
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="pending" className="mt-3">
          {filteredPending.length === 0 ? (
            <div className="py-6 text-center text-sm text-muted-foreground">Nenhuma tarefa pendente</div>
          ) : (
        ...
        <TabsContent value="completed" className="mt-3">
          {filteredCompleted.length === 0 ? (
            <div className="py-6 text-center text-sm text-muted-foreground">Nenhuma tarefa concluída</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
